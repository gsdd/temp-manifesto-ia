...
Every block on every page is one of the named components in the **Component library** (`catalogue/`): one name, one shape, one interaction type. The seven interaction types are Band, Button, Text link, Card, Tag, Filter and Static. Cards are visually distinct by type: Offer card (grey fill), Theme card and Role card (outline; Role has a top rule), Article card (no fill, no image), Case card (image placeholder), Report card (portrait document), Person card (avatar). Each interaction type has one silhouette: buttons are a heavy 2px ink frame with a light fill; text links (including pagination) are underlined or plain text, never boxed; cards are a solid 1px frame; tags are tiny squared labels; filters are pills; bands (Hero, Closing CTA band) bleed to the edge with a rule top and bottom; static blocks are dashed or plain. Home, the services hub and the Work listing carry tiny caps type annotations (`BUTTON`, `OFFER CARD`, `FILTER`) beside key blocks as a reading convention, not shipping UI. The library is generated from the same vocabulary the pages use, and the build fails if a page names a component the library does not have.
